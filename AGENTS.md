...
| goleo build ios | Build a debug iOS `GoleoApp.app` (gomobile xcframework + xcodegen + xcodebuild). Verified building, installing and running on a simulator as of 0.10.3 |
...
`backend/gomobile/{gomobile.go,notifier.go}` are GENERATED** and regenerated before every
