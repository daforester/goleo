...
The command list and the build-target matrix live in
**[`docs/agents/cli.md`](docs/agents/cli.md)** — read it before changing `cli/cmd/`. Most of it is
derivable from `goleo --help`; what is not is which targets refuse rather than warn, and what each
needs configured first. The flag-validation rules below stay here because the CI gates they
describe cover `runtime/` too.
...
`backend/init.js` is optional and does two jobs through the embedded JS engine (goja): it
drives window creation (`createWindow`/`getConfig`), and it is a scripting layer in **both**
directions — Go calls the functions it defines via `app.JS().Call(ctx, name, args...)`, and
the script reaches bridge commands via `goleo.invoke` / `goleo.emit`.

Three things about it that are not guessable and have already caused bugs:

- **The VM is owned by one goroutine.** goja is not goroutine-safe and `jsruntime.go` had no
  locking at all — safe only because `Run()` was called once at startup. Every VM access now
  goes through `submit()` in `runtime/jsruntime_call.go`. Do not touch `jsr.vm` directly.
- **`goleo.invoke` routes through `Bridge.HandleRequestContext`, never the handler map**, so
  the `Policy` ACL applies to scripts for free. A second path into that map is how an ACL
  gets holed. `HandleRequestContext` exists because the context carries the re-entrancy
  marker that stops JS→Go→JS deadlocking on the goroutine it is already running on.
- **Go→JS needs no ACL; JS→Go does.** init.js is app-author code at the same trust level as
  `backend/app/app.go`, and Go already has unrestricted access — the frontend is what
  `Policy` defends against.

The VM, both scaffolds' comment blocks and the generated `backend/init.d.ts` are held to the
same global set by tests in `runtime/jsruntime_test.go`; the scaffolds once documented a
`bridge` object the engine never defined, so all three move together or CI fails.
