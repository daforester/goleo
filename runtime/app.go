...
func (a *App) runWebview(port int) error {
	...
	go func() {
		...
		// Unblock Run so shutdown can proceed. The glaze backend (all three
		// desktops) Terminate()s the run loop from any goroutine — which glaze
		// makes safe — so Run returns.
		win.endRunLoop()
	}()
