...
func New(opts Options) *runtime.App {
	...
	a = runtime.New(runtime.Config{
		...
		OnStartup: func(ctx context.Context) {
			...
			//
			// Battery, WakeLock, Geolocation, Camera, Microphone, Clipboard,
			// Dialogs, FS, Sensors and Background work on BOTH Android and iOS,
			// via a native provider and/or a WebView browser-API fallback, and
			// are checked on real hardware.
			//
			...
			// Separate from Camera on purpose, so an app that only wants stills
			// does not ask users for RECORD_AUDIO.
			...
			//
			// NFC and BLE are ANDROID-ONLY. iOS registers no provider for
			// either and WKWebView implements neither Web NFC nor Web
			// Bluetooth, so on iOS these do not fall back — the commands report
			// that no native provider is available.
			// runtime.RegisterNFC(a.Bridge())
			// runtime.RegisterBLE(a.Bridge())
			//
			// Push additionally needs your own Firebase/Apple Developer project
			// no matter what native code exists — see
			// cli/cmd/templates/android/.../MainActivity.java for what is
...
// resolve on Windows instead of waiting on a prompt nothing answers. Go replace
// directives don't transit, so this has to be repeated here rather than being
// inherited from goleo. Dropping it still builds, but silently loses that grant
// on Windows. See goleo's AGENTS.md / SPIKES.md.
...
# Signing keys. "goleo generate android-key" writes release.jks into this directory and tells
# you not to commit it — this makes that harder to get wrong. A committed upload key lets
# anyone sign updates as you, and Play will not let you change the key a listing uses.
*.jks
*.keystore
*.p12
*.mobileprovision
...
var tmplMobileDialogsGo = `// Code generated by goleo. DO NOT EDIT.
// This file is regenerated on every goleo dev/build/emulate run.

//go:build mobilebuild && goleo_dialog
...
// DialogsProvider is implemented by the native shell (Android Activity / iOS
// AppDelegate) to show native dialogs and file pickers. gomobile generates
// bindings for this interface.
//
// Every method takes and returns a JSON STRING rather than the runtime's option
// structs, for the same reason BLEProvider.RequestDeviceJSON does: gobind cannot
// generate a reverse-bound (shell-implemented) proxy method that takes or returns
// a pointer to a struct declared in a different package than the interface, and
// it binds neither a slice of structs (FileDialogOptions.Filters) nor a []string
// result. A method gobind cannot bind is silently OMITTED from the generated
...
func SetDialogsProvider(p DialogsProvider) {
	if p == nil {
		runtime.SetDialogsProvider(nil)
		return
	}
	runtime.SetDialogsProvider(&dialogsAdapter{p: p})
}
...
var tmplMobileMicrophoneGo = `// Code generated by goleo. DO NOT EDIT.
// This file is regenerated on every goleo dev/build/emulate run.

//go:build mobilebuild && goleo_microphone

package gomobile

import "github.com/daforester/goleo/runtime"

// MicrophoneProvider is implemented by the native shell (Android Activity / iOS
// AppDelegate) to report and request microphone permission. Recording itself is
// done in the WebView with getUserMedia + MediaRecorder; only the permission
// state needs a native call. Its method set matches runtime.MicrophoneProvider
// exactly, so it satisfies that interface directly with no adapter needed.
//
// Both methods return a lone value and NO error, matching Notifier's shapes.
// That is load-bearing: for a Go (value, error) result gobind emits a _Nonnull
// return alongside error:(NSError**), which Swift will not import as a throwing
// method, so no Swift signature conforms and the iOS shell fails to build. See
// SPIKES.md, 2026-08-10.
type MicrophoneProvider interface {
	PermissionGranted() bool
	RequestPermission() string
}

// SetMicrophoneProvider registers the native microphone backend.
func SetMicrophoneProvider(p MicrophoneProvider) {
	if p == nil {
		runtime.SetMicrophoneProvider(nil)
		return
	}
	runtime.SetMicrophoneProvider(p)
}
`
